...
def f_combine(d_prod, l_ord):
    '''
Process:  Create a new list containing the each product and attributes and total order quantity for the product
...
'''
    ...
    for i in sorted(l_combine, key = operator.itemgetter(5), reverse = True):
        print(dat_fmt.format(i[0], i[1], i[2], i[3], '${:,.2f}'.format(i[4]), i[5]))

    return

# todo
# make report with input from user on how to sort
#################################################################
def main():
